...
import { uploadAdminFile } from "@/api/adminUploads";
import { fetchAdminInternalTicketById, replyToInternalTicketAsAdmin, resolveInternalTicket } from "@/api/admin";
...
export function AdminTicketDetailPage() {
  ...
  const upload = useFileUpload(uploadAdminFile);

  const { data, isLoading } = useQuery({
    queryKey: ["adminInternalTicket", id],
    queryFn: () => fetchAdminInternalTicketById(id as string),
    enabled: Boolean(id),
  });
  ...
  function invalidate() {
    queryClient.invalidateQueries({ queryKey: ["adminInternalTicket", id] });
    queryClient.invalidateQueries({ queryKey: ["internalTickets"] });
  }

  const replyMutation = useMutation({
    mutationFn: () => replyToInternalTicketAsAdmin(id as string, { message, attachments: upload.attachments }),
    onSuccess: () => {
    ...
    onError: (err) => {
      toast.error(err instanceof AdminApiError ? err.message : "Não foi possível enviar a resposta.");
    },
  });

  const resolveMutation = useMutation({
    mutationFn: () => resolveInternalTicket(id as string),
    onSuccess: () => {
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Aberto em {formatDateTime(ticket.createdAt)} por {ticket.author.name} ({ROLE_LABEL[ticket.author.role]}
            {ticket.author.cargo ? ` · ${ticket.author.cargo}` : ""} · {ticket.author.setor})
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            >
              ...
              <Button type="submit" disabled={replyMutation.isPending || upload.isUploading} className="self-start">
                Enviar resposta
              </Button>
            </form>
          </CardContent>
